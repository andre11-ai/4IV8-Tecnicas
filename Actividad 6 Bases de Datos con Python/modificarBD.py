...
try:
    ...
    cla = input("clave de unidada de aprendizaje a modificar")
    nom = input("Nombre por el que quiere sustituir")

    # Peligro inyección SQL    query = "UPDATE malumno SET nom_alu = '"+nombre+"' WHERE bol_alu = '"+boleta+"'"
    query = "UPDATE munidadaprendizaje SET nom_ua = %s WHERE cla_ua = %s"
    valores = (nom, cla)
